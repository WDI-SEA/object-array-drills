...
if (band.albums.length % 2 === 0) {
  console.log(`The number of albums ${band.albums.length} is even`)
} else {
  console.log(`The number of albums ${band.albums.length} is odd`)
}

if (band.albums.length % 2 === 0 && band.albums.length % 3 === 0) {
  console.log(`The number of albums ${band.albums.length} is divisible by 2 and 3`)
} else if (band.albums.length % 2 === 0) {
  console.log(`The number of albums ${band.albums.length} is is divisible by 2`)
} else if (band.albums.length % 3 === 0) {
  console.log(`The number of albums ${band.albums.length} is is divisible by 3`)
} else {
  console.log(`The number of albums ${band.albums.length} is is divisible by neither 2 or 3`)
}
...
//   if (band.albums[i].albumDetails.labels.indexOf('Sire') !== -1 || band.albums[i].albumDetails.labels.indexOf('sire') !== -1 || band.albums[i].albumDetails.label === "Sire" || band.albums[i].albumDetails.label === "sire") {
//     sireTally++
//   }
...
for (let i = 0; i < ticketSections.length; i++) {
  if (ticketSections[i].type === "premium" && ticketSections[i].seats > 1) {
  ...
  } else if (ticketSections[i].type === "premium") {
    ...
    console.log(`Welcome, ${ticketSections[i].name}! you and your party may sit anywhere except the first 3 rows of the ${ticketSections[i].section} section.`)
  }
...
// loop through object -> if discount and zombie deduct cost print cost
// ran out of time, knew what i had to do here.