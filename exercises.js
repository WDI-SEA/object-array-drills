...
// if key doesn't exist in the object, you can just use dot notation to add it
...
console.log(album5.albumDetails.format)
...
// assign a variable to make it easier
// so you can say numAlbums = talkingHeadsAlbums.length
if (talkingHeadsAlbums.length % 2 === 0) {
  console.log("The number " + talkingHeadsAlbums.length + " is even")
} else {
  "The number " + talkingHeadsAlbums.length + " is odd"
}
...
talkingHeadsAlbums.forEach(album => {
  ...
  // check if it exists
    ...
    if (album.albumDetails.label.includes("Sire")) { // includes is an array method, also works for strings!
      sireTally++
    }
  }
})
