...
const labelString = album1.albumDetails.label
...
album3.albumDetails.released = new Date("April 27, 1993")
...
album5.albumDetails.formats = ['CD','Cassettes','LP']
...
const band = {
  name: "Talking Heads",
  members: ["David Byrne", "Tiny Weymouth","Chris Franz", "Jerry Harrison"],
...
if (band.albums.length % 2 === 0) {
  ...
  console.log(`The number ${band.albums.length} not divisble by 2 or 3`)
}

// // 4. Check your logic above against the numbers: 0, 1, 2, 6, 7, and 9.
// //    Make sure it always works!

if (band.albums.length % 9 === 0) {
  console.log(`The number ${band.albums.length} is divisible 9`)
} else { 
  console.log(`The number ${band.albums.length} not divisble 9`)
}
...
sireTally = 0
for (let i=0; i < talkingHeadsAlbums.length;i++){
  if (talkingHeadsAlbums[i].albumDetails.label == 'Sire' || talkingHeadsAlbums[i].albumDetails.labels[0] == 'Sire'){
    sireTally++
  }
}
console.log(sireTally) // 7 
...
ticketSections.forEach((ticketSection)=> {
  if (ticketSection.seats < 3){
    console.log (`Welcome, ${ticketSection.name}! You may sit anywhere in the first 3 rows of the ${ticketSection.section} section.`)
  } else {
    console.log (`Welcome, ${ticketSection.name}! You and your party may sit anywhere except first 3 rows of the            
...
const tickets = [
  {amount: 50.00, discount: false, zombie: true}, // $10 voucher                            // NOT SURE BOUT DIS ONE!
  {amount: 60.00, discount: true,  zombie: false}, //Premier $10 drink
  ...
  {amount: 50.00, discount: true,  zombie: false},  //error
  {amount: 50.00, zombie:   true},                   //standard $10 drinks
  {amount: 80.00, discount: true},                    // preimier plus 
  {amount: 90.00},                                    // premier plus
  {amount: 50.00, discount: true}                      // error
]  

tickets.forEach((ticket)=>{
  if (ticket.amount >= 80){
    console.log(ticket,'PREMIER PLUS')
  } else if (ticket.amount < 80 && ticket.amount >= 60 && ticket.discount === true && ticket.zombie === true ){
    console.log(ticket, 'PREMIER $20 DRINKS')
  } else if (ticket.amount < 80 && ticket.amount >= 60 &&  ticket.discount === true && ticket.zombie === false ){
      console.log (ticket,'PREMIER $10 DRINKS')
  } else if (ticket.amount < 60 &&  ticket.discount === false && ticket.zombie === true ){
      console.log (ticket,'STANDARD $10 DRINKS')
  } else if (ticket.amount < 60 &&  ticket.discount === true && ticket.zombie === false ){
      console.log (ticket,'STANDARD NO DRINKS')
  } else if (ticket.amount === 50 &&  ticket.discount === true){ 
