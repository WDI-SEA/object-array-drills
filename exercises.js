...
//    variable.
let name = album1.albumDetails.label
...
let put = album3.albumDetails.formats
let take = album2.albumDetails.formats[0]
...
//doesnt edit the original object ????????????????????????????????????????
...
let labelArray = album7.albumDetails.labels.split();
album7.albumDetails.labels = labelArray;
...
}else if(band.albums.length % 2 === 1){
...
// let sireTally = 0;
// console.log(talkingHeadsAlbums[1].albumDetails.label)
// for(let i = 0; i < talkingHeadsAlbums.legnth; i++){
//  if(talkingHeadsAlbums[i].albumDetails.lebel === "Sire"){
//    console.log("yes")
//  }
// }
...
for (let i = 0; i < band.albums.length; i++) {
  ...
  if(band.albums[i].albumDetails.labels) {
    for(let j = 0; j < band.albums[i].albumDetails.labels.length; j++) {
      if(band.albums[i].albumDetails.labels[j] === "Sire") {
        sireTally++
      }
    }
  }
...
for(let i = 0; i < tickets.length; i++){
  ...
  else if(tickets[i].amount === 60.00 || (tickets[i].amount === 80.00 && tickets[i].discount === false)){
  console.log("ERROR: INVALID TICKET");
    ...
    if(tickets[i].discount === true){
      console.log("PREMIER PLUS IS $80");
    }else {
        console.log("PREMIER PLUS");
    }
  }else if(tickets[i].amount === 80.00 && tickets[i].discount === true){
    console.log("PREMIER PLUS IS $80");
  }
}
