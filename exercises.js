...
const newLabel = album1.albumDetails.label
...
album3.albumDetails.formats.push(
...
//SHOWS UP AS LOWERCASE STILL...needed to overwrite old version
...
// 3. Give it the property `members` and set it to an array with a single
//    string, "David Byrne", in it.
band.members = ["David Byrne", "Tiny Weymouth", "ChrisFranz", "Jerry Harrison"]
// 4. Give it the property `albums` and set it to the array stored in the
//    variable talkingHeadsAlbums
...
band.members.push("Tiny Weymouth", "ChrisFranz", "Jerry Harrison")
...
} else console.log("Talking heads didn't have much output")
...
if (numAlbums % 2 != 0) //check if its even
  {
    console.log("the number " + talkingHeadAlbums.length + " is odd")
    } else {
...
const y = numAlbums
if(y % 2 != 0 && y % 3 != 0) {
  console.log('the number is not div by two or three')
} else if (y % 2 === 0 && y % 3 === 0) {
  console.log('tje number is div by two or three')
} else if (y%3===0) {
  console.log('the number is div by three')
} else if (y%2===0) {
  console.log('the number is div by two')
}
...
let sireTally = 0
talkingHeadAlbums.forEach(album => {
  ...
  console.log(album.albumDetails.label)
    ...
    console.log('it has labels with an s')
    }
  }
})
