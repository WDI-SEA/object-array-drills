...
//Answer
album4.albumDetails.label = recordLabel
...
// 1. Create an object literal called `band`. / 2. Give it the property `name` and set it to "Talking Heads" / 3. Give it the property `members` and set it to an array with a single
//    string, "David Byrne", in it. / 4. Give it the property `albums` and set it to the array stored in the
...
//Answer
if (talkingHeadsAlbums.length >= 6) { console.log ("Talking heads were a prolific band.")
} else {
...
if (talkingHeadsAlbums.length % 2 !=0) { console.log("The number" + talkingHeadsAlbums.length + "is odd") 
} else {console.log("The number" +talkingHeadsAlbums.length + "is even")
}
...
if (talkingHeadsAlbums.length % 2 === 0) {
  ...
  console.log("The number is divisble by two or three")
...
for (let i = 1; i< talkingHeadsAlbums.length; i++){
  ...
  talkingHeadsAlbums.forEach(album => {
    if (album.albumDetails.label === "Sire") {
      ...
      if(album.albumDetails.labels.includes('Sire')) { sireTally++
      }
    }
/////////////////////////////////////////////////////
...
//Answer 
if (ticket1.type === "premium") {
  console.log("Welcome, " + ticket1.name + "! You may sit anywhere in the first 3 rows of the right section.") 
  } else console.log("Welcome, " + ticket1.name + "! You and your party may sit anywhere except first 3 rows of the center section.Please be sure to leave no seats between you.") 
...
//Answer
const names = ticketSections.map((name1, index, array)=> {
  return name1.name
  }) 
  console.log(names)
...
//Answer
  tickets.forEach((tix) => {
    if(tix.amount === 50){
      if(tix.discount === true && tix.zombie === true){
        console.log('STANDARD - $20 DRINKS')
      }else if(tix.discount === true || tix.zombie === true){
        console.log('STANDARD - $10 DRINKS')
      }else console.log('STANDARD NO DRINKS')
    }else if(tix.amount === 65){
      if(tix.discount === true && tix.zombie === true){
        console.log('PREMIER - $20 DRINKS')
      }else if(tix.discount === true || tix.zombie === true){
        console.log('PREMIER - $20 DRINKS')
      }else console.log('PREMIER NO DRINKS')
    }else if(tix.amount === 80){
      if(tix.discount === true && tix.zombie === true){
        console.log('PREMIER PLUS - $20 DRINKS')
      }else if(tix.discount === true){
        console.log('PREMIER PLUS - DISCOUNTED - $10 DRINKS')
      }else console.log('ERROR: INVALID TICKET')
    }else if(tix.amount === 90){
      if(tix.zombie === true){
        console.log('PREMIER PLUS - $10 DRINKS')
      }else console.log('PREMIER PLUS - NO DRINKS')
    }else console.log('ERROR: INVALID TICKET')
  })
``