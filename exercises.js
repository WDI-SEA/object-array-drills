...
//    variable.
const sireTrack = album1.albumDetails.label
console.log("1. ", album1.albumDetails.label)
// 2. Change the title of album1 from "Talking Heads" to "Talking Heads: 77"
album1.title = "Talking Heads: 77"
console.log("2. ", album1.title = "Talking Heads: 77")
...
if (talkingHeadsAlbums.length % 2 != 0) {
  ...
  console.log(`2. The number ${talkingHeadsAlbums.length} is even with ${talkingHeadsAlbums.length} being the number of albums`)
}
...
let albumNums = talkingHeadsAlbums.length
albumNums = 6
if (albumNums % 2 === 0) {



  console.log(`3. The ${albumNums} is divisible by 2`)
} else if (albumNums % 3 === 0) {
  console.log(`3. The ${albumNums}  is divisible by 3`)
} else if (albumNums % 2 === 0 && albumNums % 3 === 0) {
  console.log(`3. The ${albumNums}  is divisible by 2 and 3`)
} else {
  console.log(`3. The ${albumNums}  is not divisible by 2 or 3`)
}
...
for (let i = 0; i < ticketSections.length; i++) {
  if (ticketSections[i].type === "premium") {
    console.log(`Welcome,${ticketSections.name}! You may sit anywhere in the first 3 rows of the right section`)
  } else if (ticketSections[i].type === "standard") {
    console.log(`Welcome,${ticketSections.name} You and your party may sit anywhere except first 3 rows of the center section`)
...
for (let i = 0; i < tickets.length; i++) {
  ...
  if (tickets[i].discount === true) {
    //saying voucherValue = voucherValue + 10
    voucherValue += 10
  }
  if (tickets[i].zomnie === true) {
    voucherValue += 10
  }
  if (tickets[i].amount === 50) {
    // console.log("STANDARD")
    //ternary 3 part 
    //conditional question true case : false case
    voucherValue > 0 ? console.log(`STANDARD $${voucherValue} Drinks`) :
      //Conditional ?          True Case            
      console.log("STANDARD NO DRINKS")
    //false case
  } else if (tickets[i].amount === 65) {
    // console.log("PREMIERE")
    voucherValue > 0 ? console.log(`PREMIERE $${voucherValue} DRINKS`) :
      console.log("REMIERE NO DRINKS")
