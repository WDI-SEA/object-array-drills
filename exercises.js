...
albumOneLabel = "Talking Heads: 77";
console.log(albumOneLabel);
...
let album3date = new Date("October 31, 1979");
album3.albumDetails.released = album3date
console.log(album3)
...
album7.albumDetails.labels = album7.albumDetails.labels.split(comma);
console.log(album7);
...
console.log(band.albums.length >= 6 ? `${band.name} were a proflific band` : `${band.name} didn't have much output.`);
...
console.log(band.albums.length %2 === 0 ? `The number ${band.albums.length} is even` : `The number ${band.albums.length} is odd`);
...
console.log(band.albums.length %3 === 0 ? `The number ${band.albums.length} is divisible by 3` : `The number ${band.albums.length} is not divisible by 3`);


// 4. Check your logic above against the numbers: 0, 1, 2, 6, 7, and 9.
//    Make sure it always works!
console.log(band.albums.length %9 === 0 ? `The number ${band.albums.length} is divisible by 9` : `The number ${band.albums.length} is not divisible by 9`);
...
talkingHeadsAlbums.forEach(function (album) {
  if (talkingHeadsAlbums.label = "Sire") {
    sireTally++;
  } else if (talkingHeadsAlbums.labels[i] = "Sire") {
    sireTally++;
  } else {}
})
