...
for (let i = 0; i < band.albums.length; i++) {
  console.log("Album name: " + band.albums[i].title);
}
...
let sireTally = 0;
for (let i = 0; i < band.albums.length; i++) {
  if (Array.isArray(band.albums[i].albumDetails.labels) 
  && (band.albums[i].albumDetails.labels.indexOf("Sire")>=0)) {
    sireTally++;
...
for (let i = 0; i < ticketSections.length; i++) {
  if (ticketSections[i].type === "premium") {
    console.log("Welcome, " + ticketSections[i].name + "! You may sit anywhere in the first 3 rows of the " 
    + ticketSections[i].section + " section.");
  }else {
    console.log("Welcome, " + ticketSections[i].name + "! You and your party may sit anywhere except first 3 rows of the " 
    + ticketSections[i].section + " section. Please be sure to leave no seats between you.");
  }
}
...
for (let i = 0; i < tickets.length; i++) {
  if (tickets[i].amount === 90 && tickets[i].zombie) {
    console.log("PREMIER PLUS $10 DRINKS");
  }else if (tickets[i].amount === 80 && tickets[i].discount && tickets[i].zombie) {
    console.log("PREMIER PLUS $10 DRINKS");
  }
