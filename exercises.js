...
for(let i = 0; i < ticketSections.length; i++){
  let guest = ticketSections[i];
  if(guest.type === "premium" && guest.seats === 1){
    console.log(`Welcome ${guest.name}! You may sit anywhere in the first 3 rows of the ${guest.section} section. Please be sure to leave no seats between you.`)
  } else if(guest.type === "premium" && guest.seats > 1){
    console.log(`Welcome ${guest.name}! You and your party may sit anywhere in the first 3 rows of the ${guest.section} section. Please be sure to leave no seats between you.`)
  } else if(guest.type === "standard" && guest.seats === 1){
    console.log(`Welcome ${guest.name}! You may sit anywhere except first 3 rows of the ${guest.section} section. Please be sure to leave no seats between you.`)
  }else if(guest.type === "standard" && guest.seats > 1){
    console.log(`Welcome ${guest.name}! You and your party may sit anywhere except first 3 rows of the ${guest.section} section. Please be sure to leave no seats between you.`)
  } else {
     console.log("You don't have tickets bud.")   
  }
}
...
for(let i = 0; i < tickets.length; i++) {
    ...
    let voucher = 0 
    let ticketType = "" 
    if(currentTicket.zombie) {
        voucher += 10 
    }
    if(currentTicket.discount) {
        voucher += 10 
    }
    if(currentTicket.amount == 50) {
        ticketType = "STANDARD" 
    } else if(currentTicket.amount == 65) {
        ticketType = "PREMIER" 
    } else if (currentTicket.amount == 80 && currentTicket.discount) {
        console.log("PREMIER PLUS") 
        continue 
    } else if (currentTicket.amount == 90) {
        console.log("PREMIER PLUS") 
        continue 
    } else {
