...
releaseDate = album3.albumDetails.released
releaseDate = new Date('1979-08-03')
...
let newLabels = album6.albumDetails.labels[1].toUpperCase()
album6.albumDetails.labels[1] = newLabels
...
let newArr = album7.albumDetails.labels.split(', ')
album7.albumDetails.labels = newArr
...
for (let i=0;i<talkingHeadsAlbums.length;i++) {
  if (talkingHeadsAlbums[i].albumDetails.label = 'Sire') {      // worked when changed from == to = but don't think this is good practice
    console.log("sire count working")
    sireTally = sireTally + 1
  } else if (talkingHeadsAlbums[i].albumDetails.labels = 'Sire' ) {
    console.log("sire count working")
    sireTally = sireTally + 1
  } else {
    console.log("sire count not working")
  }
}
...
for (let i = 0; i<ticketSections.length; i++) {
  if (ticketSections[i].type == 'standard') {
    console.log('Welcome, ' +ticketSections[i].name+ '! You and your party may sit anywhere except the first three rows of the ' +ticketSections[i].section+ ' section.')
  } else {
...
const tickets = [
  ...
  {amount: 65.00, discount: true,  zombie: false},
  {amount: 50.00},
...
for (let i=0; i<tickets.length; i++) {
  ...
  if (tickets[i].amount == 50.00) {
    ...
    } else if (tickets[i].zombie) {
      console.log('Standard $10 drinks for zombie costume')
    } else {
      console.log('Standard ticket, no drink money')
    }
  }
  // premier ticket conditional
    ...
    if (tickets[i].discount) {
      ...
      console.log('Premier $10 drinks for zombie costume')
    } else {
      console.log('Premier ticket, no drink money')
    }
  }
  //premier plus conditional
