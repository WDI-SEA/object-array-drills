...
if (band.albums.length >= 6) {
  ...
  console.log('Talking Heads didnt\'t have much output')
}
...
if (y % 2 === 0 && y % 3 === 0) {
  ...
  console.log('The number ' + y + ' is not divisible by 2 nor 3')
}
...
for (let taco of tickets){
  let ticket = ""
  let drink = 0

  //tracks drink credit
  if (taco.discount == true) {
    drink += 10
  }
  if (taco.zombie == true){
    drink += 10
  ...
  //different seating tiers for different prices
  if (taco.amount === 50.00){
    ticket ="STANDARD "
  } else if (taco.amount === 65.00){
    ticket = "PREMIUM "
  } else if (taco.amount === 90.00){
    console.log("PREMIER PLUS")
    continue
  } else if (taco.amount === 80.00 && taco.discount == true){
    console.log("PREMIER PLUS")
    continue
  } else {
