...
let currentLabel = album6.albumDetails.labels[1]
album6.albumDetails.labels[1] = currentLabel.toUpperCase()
...
if(band.albums.length >= 6){
  ...
  "Talking heads didn't have much output."
}
...
let albumCount = band.albums.length
if(albumCount % 2 === 0){
  console.log(`The number ${albumCount} is even`)
}else{
  console.log(`The number ${albumCount} is odd`)
}
...
if(albumCount % 2 === 0 || albumCount % 3 === 0){
  if(albumCount % 2 === 0){
    if(albumCount % 3 === 0){
...
talkingHeadsAlbums.forEach((element)=>{
  ...
  sireTally++
  }else if(element.albumDetails.labels){
    if(element.albumDetails.labels){
      if(element.albumDetails.labels[0] === "Sire"){
        sireTally++
      }else if(element.albumDetails.labels.includes("Sire")){
        sireTally++
      }
    }
  }
...
ticketSections.forEach((element)=>{
  let seatingSection = element.section
  let ticketType = element.type
  let ticketHolder = element.name
  let seatCount = element.seats
  //if premium you get front 3 rows.
  //asign section
  if(ticketType = "premium"){
    console.log(`Welcome, ${ticketHolder} You may sit anywhere in the first 3 rows of the ${seatingSection} section.`)
  }else if(ticketType = "standard"){
    console.log(`Welcome, ${ticketHolder} You and your party may sit anywhere except first 3 rows of the ${seatingSection} section.`)
  }
...
tickets.forEach((ticket)=>{
  let price = Math.trunc(parseInt(ticket.amount))
  let ticketText = ""
  let drinkCredit = 0;
  //three prices: 50,65,90
  if(price === 50){
    ticketText += "STANDARD"
  }else if(price===65){
    ticketText += "PREMIER"
  }else if(price===90){
    ticketText += "PREMIER PLUS"
  }else if(price === 80 && ticket.discount){
    ticketText += "PREMIER PLUS"
    drinkCredit += 10
  }else{
  ...
  if(ticketText){
    console.log(ticketText + ' $' + drinkCredit + ' DRINKS')
  }
})