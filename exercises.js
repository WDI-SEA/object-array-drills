...
album7.albumDetails.labels = Array.from(album7.albumDetails.labels.split(", "));
...
if (
  talkingHeadsAlbums.length % 2 === 0 &&
  talkingHeadsAlbums.length % 3 !== 0
) {
  ...
  console.log(
    `The number ${talkingHeadsAlbums.length} is divisible by 2 and 3`
  );
} else {
  console.log(
    `The number ${talkingHeadsAlbums.length} is not divisible by 2 or 3`
  );
}
...
for (let i = 0; i < ticketSections.length; i++) {
  if (ticketSections[i].type === "premium" && ticketSections[i].seats === 1) {
    console.log(
      "Welcome, " +
        ticketSections[i].name +
        "! You may sit anywhere in the first 3 rows of the " +
        ticketSections[i].section +
        " section."
    );
  } else if (
    ticketSections[i].type === "standard" &&
    ticketSections[i].seats === 1
  ) {
    console.log(
      "Welcome, " +
        ticketSections[i].name +
        "! You may sit anywhere except the first 3 rows of the " +
        ticketSections[i].section +
        " section."
    );
  } else if (
    ticketSections[i].seats > 1 &&
    ticketSections[i].type === "premium"
  ) {
    console.log(
      "Welcome, " +
        ticketSections[i].name +
        "! You may sit anywhere in the first 3 rows of the " +
        ticketSections[i].section +
        " section.Please be sure to leave no seats between you."
    );
  } else if (
    ticketSections[i].seats > 1 &&
    ticketSections[i].type === "standard"
...
const std = "STANDARD";
const prem = "PREMIER";
const premPlus = "PREMIER PLUS";
const error = "ERROR: INVALID TICKET";
