...
// I'm assuming the formats array needs to be in the albumDetails object, but that's not explicitly stated by the question
...
talkingHeadsAlbums.length >= 6 ? console.log("Talking Heads were a prolific band") : "Talking heads didn't have much output."
...
numberOfAlbums % 2 === 0 ? console.log(`The number ${numberOfAlbums} is even`) : console.log(`The number ${numberOfAlbums} is odd`)
...
for (let i = 0; i < talkingHeadsAlbums.length; i++) {
  ...
  if (talkingHeadsAlbums[i].albumDetails.labels && talkingHeadsAlbums[i].albumDetails.labels.includes("Sire")) {
    sireTally++
  }
}
