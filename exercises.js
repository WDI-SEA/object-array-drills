...
//    variable.
let labelSire = album1.albumDetails.label
// console.log(album1)
...
let formatLP = album2.albumDetails.formats[0]
album3.albumDetails.formats.push(formatLP)
// console.log(album3)
...
// let label2 = emi.toUpperCase()
// album6.albumDetails.labels[1] = label2
console.log(album6)
...
let stringLabels = album7.albumDetails.labels
let arrayLabels = stringLabels.split(",")
// console.log(arrayLabels)
album7.albumDetails.labels = arrayLabels
// console.log(album7)
/////////////////////////////////////////////////////
...
switch(true) {
  case (albumNum % 2 === 0 && albumNum % 3 !==0):
    console.log("The number " + albumNum + " is divisible by 2")
    break
  case (albumNum % 3 === 0 && albumNum % 2 !==0):
...
logicCheck = [0, 1, 2, 6, 7, 9]
for (i = 0; i < logicCheck.length; i++) {
  switch(true) {
  case (logicCheck[i] % 2 === 0 && logicCheck[i] % 3 !==0):
    console.log("The number " + logicCheck[i] + " is divisible by 2")
    break
  case (logicCheck[i] % 3 === 0 && logicCheck[i] % 2 !==0):
    console.log("The number " + logicCheck[i] + " is divisible by 3")
    break
  case (logicCheck[i] % 2 === 0 && logicCheck[i] % 3 ===0):
    console.log("The number " + logicCheck[i] + " is divisible by 2 and 3")
    break
  case (logicCheck[i] % 2 !== 0 && logicCheck[i] % 3 !==0):
    console.log("The number " + logicCheck[i] + " is not divisible by 2 or 3")
    break
  }
}
/////////////////////////////////////////////////////
...
for (let i = 0; i < band.albums.length; i++) {
  if (band.albums[i].albumDetails.labels) {
    if (band.albums[i].albumDetails.labels.includes("Sire")){
    sireTally++
    }
...
//can make 1st if a conditional with && instead of nesting
...
for (i = 0; i < ticketSections.length; i++) {
  ...
  if (ticketSections[i].type === "premium") {
    seatingType = "may sit anywhere in the first 3 rows of the"
  }else {
    seatingType = "may sit anywhere except the first 3 rows of the"
  }
  if (ticketSections[i].section === "right") {
    section = "right section"
  } else if (ticketSections[i].section === "center") {
    section = "center section"
  } else {
    section = "left section"
  }
  if (ticketSections[i].seats > 1) {
    partySize = "You and your party"
    commonCourtesy = "Please be sure to leave no seats between you."
  }
  console.log("Welcome,", ticketSections[i].name + "!", partySize, seatingType, section + ".", commonCourtesy)
}
...
for (i = 0; i < tickets.length; i++) {
  ...
  } else if (tickets[i].amount === 80.00 && tickets[i].discount){
    ticketType = "PREMIER PLUS"
  } else {
    ticketType = "ERROR: INVALID TICKET"
  }
  if (drinkDisc > 0 && ticketType !== "ERROR: INVALID TICKET") {
    ticketType += (" $" + drinkDisc + " DRINKS") 
  } else if (drinkDisc === 0 && ticketType !== "ERROR: INVALID TICKET"){
    ticketType += (" NO DRINKS")
  }
